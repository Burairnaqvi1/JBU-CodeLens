...
using MethodDetailContext = CodeLensAI.Core.Structural.MethodDetailContext;
...
internal static class DetailPanelRenderer
{
        ...
        out TextBlock? aiBriefText)
    {
        ...
        else
        {
            ...
            var inferred = MethodDescriptionBuilder.Build(method);
    ...
    private static Border BuildVariablesCard(MethodDetailContext context, FrameworkElement resourceRoot)
    {
        ...
        if (method.OperationalLimits.Count > 0)
        {
            stack.Children.Add(new Border { Height = 1, Background = Brush(resourceRoot, "BorderBrush"), Margin = new Thickness(0, 12, 0, 10) });
            stack.Children.Add(CreateCapsLabel("OPERATIONAL LIMITS", resourceRoot));
            foreach (var limit in method.OperationalLimits)
                stack.Children.Add(CreateBulletItem(OperationalLimitFormatter.Format(limit, method), resourceRoot));
        }
    ...
    private static Border BuildErrorsCard(MethodDetailContext context, FrameworkElement resourceRoot, ExplanationService? explanationService)
    {
        var method = context.Method;
        var stack = new StackPanel();
        ...
        TextBlock? aiBriefText)
    {
        ...
        MethodConversationSession? session = null;
    ...
    private static string GetAiUnavailableMessage(ExplanationService? explanationService)
    {
        ...
        {
            return ModelPathResolver.ModelNotFoundMessage;
        }
